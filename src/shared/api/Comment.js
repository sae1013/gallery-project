    ...
    return (dispatch,getState,{history}) => {
        ...
        commentDB.add(newComment)
            .then((doc)=>{ // commentDB에 댓글 저장 성공시
                ...
                postDB.doc(postId).update({commentCount:increment}) // postDB의 comment 갯수를 1증가시켜야함
                    .then(()=>{
                        ...
                        if(post){
                            ...
                            notiListRef.set({ // 필요한 데이터 : 댓글내용,포스트아이디,댓글작성 한 사람닉네임,댓글작성한사람 프로필이미지, 코멘트 아이디,
            ...
            .catch(()=>{ // commentDB에 댓글 저장 실패시 에러처리
